...
async function returnBinaryData(items, responseData) {
    ...
    const binaryData = await this.helpers.prepareBinaryData(responseData.body, `audio.${operation}.mp3`, 'audio/mp3');
    return items.map(() => ({ json: responseData.headers, binary: { ['data']: binaryData } }));
}
async function preSendSpeechToText(requestOptions) {
    const binaryInputField = this.getNodeParameter('file', 'data');
    ...
    const model_id = additionalOptions.model || 'scribe_v1';
    const language_code = additionalOptions.languageCode;
    const num_speakers = additionalOptions.numberOfSpeakers;
    const diarize = additionalOptions.diarize;
...
async function preSendSpeechToSpeech(requestOptions) {
    const binaryInputField = this.getNodeParameter('file', 'data');
    const additionalOptions = this.getNodeParameter('additionalOptions', {});
    const fileBuffer = await this.helpers.getBinaryDataBuffer(binaryInputField);
    const formData = new FormData();
    formData.append('audio', new Blob([fileBuffer]));
    formData.append('model_id', additionalOptions.model || 'eleven_english_sts_v2');
    const voiceSettings = additionalOptions.voiceSettings || '{}';
